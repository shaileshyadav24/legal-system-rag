...
- **`services/config.py`** — env-driven settings (`MONGODB_URI`, `MONGODB_DB_NAME`, `JWT_SECRET`, `JWT_ALGORITHM`, `JWT_EXPIRE_MINUTES`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `PASSWORD_RESET_URL`), loaded via `python-dotenv`. Every one of these (aside from `JWT_ALGORITHM`, which is fixed) is required — `_require()` raises at import time if any are missing, no silent defaults. See `.env.example`.
...
- **`services/password_reset_service.py`** — forgot-password flow: `create_reset_token` (only stores a SHA-256 hash of the raw token, in `password_reset_tokens`, TTL 30 min, then emails the raw token via `email_service.send_password_reset_email`) and `reset_password` (validates hash/expiry/unused, then calls `update_password`). Backs `POST /auth/forgot-password` / `POST /auth/reset-password`. The raw token is never returned in the API response — it's only ever available inside `create_reset_token`, which immediately emails it.
- **`services/email_service.py`** — sends the password-reset email via [Resend](https://resend.com) (`RESEND_API_KEY`/`RESEND_FROM_EMAIL`), linking to `PASSWORD_RESET_URL?token=<raw_token>` (the FE's reset-password page, reading `token` from the query string). Raises a `500` (caught by FastAPI's default handler) on Resend delivery failure — deliberately not swallowed, since silently claiming success when the email didn't send would leave the user stuck.
