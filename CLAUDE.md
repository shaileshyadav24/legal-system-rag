...
Persistence is MongoDB (Atlas required — Atlas Vector Search is used for retrieval and doesn't work against a plain `mongod`; see "Local testing" below for a workaround). `services/db.py` owns the single `pymongo.MongoClient` and six collections: `users`, `chat_sessions`, `chat_messages`, `case_law_documents`, `revoked_tokens`, `password_reset_tokens`.

Request flow for a query: `app.py` → `services/api.py` (auth-gated) → `services/retrieval.py` (Atlas `$vectorSearch`) → `services/chat_service.py` (load/save history) → `services/llm.py` → `services/response_utils.py`.
...
- **`services/api.py`** — the two query endpoints, `POST /query/user` and `POST /query/lawyer`, both behind `Depends(get_current_user)`. Both funnel through `_handle_query`, which differs only by the `role` string passed to prompt building, and now also creates/loads the chat session, persists the turn, and returns `session_id` to the client. Maps retrieval/generation failures to HTTP 404 (unknown `collection_name`), 204 (no context found), 500 (generation failure).
- **`services/retrieval.py`** — embeds the query once with `DefaultEmbeddingFunction` (must match what `dataset/dataset.py` ingested with) and runs a single `$vectorSearch` aggregation against `case_law_documents`, optionally filtered by `dataset` when `collection_name` is given. Only ever returns the single best-matching chunk — the LLM never sees more than one chunk of context, which is why prompts (`prompts/prompts.py`) explicitly forbid inventing facts beyond it.
...
- **`services/history.py`** — pure formatting: turns the list of recent Mongo chat messages (from `chat_service.load_recent_messages`) into the text block injected into the prompt.
